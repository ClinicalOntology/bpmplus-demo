...
public final class DMNRuntimeUtil {
    ...
    public static DMNRuntime createRuntime(final String resourceName, final Class testClass) {
        ...
        ReleaseId ri  = ks.newReleaseId("org.kie", "dmn-test-"+UUID.randomUUID(), "1.0");
        Resource r = ks.getResources().newClassPathResource(resourceName, testClass);
    ...
    public static String formatMessages(final List<DMNMessage> messages) {
        return null;//messages.stream().map(Object::toString).peek(m -> LOG.debug(m)).collect(Collectors.joining("\n"));
    }
